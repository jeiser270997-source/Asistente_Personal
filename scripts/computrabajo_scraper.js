 ...
 * Scraper de ofertas QA/Tech en Computrabajo Colombia usando Playwright.
 * Busca "QA", "Quality Assurance", "Testing", "Playwright" en Medellín y remoto.
 * Guarda resultados en data/jobs/computrabajo.json y notifica por Telegram.
 * 
 * Loop audit #3 — faltaba completamente
 */
...
const LAST_PATH = path.join(JOBS_DIR, 'computrabajo_last.json');
...
async function loginComputrabajo(page) {
  ...
  try {
    ...
    log(ok ? '   ✅ Login OK' : '   ⚠ Login sin confirmar (continuando sin sesión)');
  } catch (e) {
...
async function scrapeSearch(page, query, label) {
  ...
  const offers = await page.evaluate((lbl) => {
    // Estructura real Computrabajo 2025
    const results = [];
    
    // Buscar artículos de oferta
    ...
    // Fallback: cualquier link de oferta
    if (results.length === 0) {
...
async function main() {
  ...
  for (const search of SEARCHES) {
    try {
      ...
      await page.waitForTimeout(1500); // rate limiting cortés
    } catch (e) {
  ...
  fs.writeFileSync(LAST_PATH, JSON.stringify({ ids: unique.map(o => o.id) }));

  log(`\n✅ ${unique.length} ofertas únicas (${nuevas.length} nuevas)`);

  // Notificar Telegram si hay nuevas
  if (nuevas.length > 0) {
    ...
    log('Sin nuevas ofertas desde la última consulta');
  }
