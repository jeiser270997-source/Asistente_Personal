...
.github/workflows/brain_orchestrator.yml
...
[
  ...
  {
    "id": "mr4tq6hbm3oq",
    "text": "Completar examen de nivelaciÃ³n y fundamentos JS/TS (tipos, coerciÃ³n, == vs ===)",
    "category": "auto",
    "done": false,
  ...
  {
    "id": "mr4tq6hc8t4p",
    "text": "Monitorear respuesta SIMIT a DP del 02/07 (C29 ItagÃ¼Ã­ 0000838097)",
    "category": "auto",
    "done": false,
...
[
  ...
  {
    "id": "mr4tq6hbm3oq",
    "text": "Completar examen de nivelación y fundamentos JS/TS (tipos, coerción, == vs ===)",
    "category": "auto",
    "done": false,
    "created": "2026-07-03T11:02:08.207Z"
  },
  {
    "id": "mr4tq6hcxe9r",
    "text": "Ejecutar bloque DDi AM 5-9 y PM 5-8",
    "category": "auto",
    "done": false,
    "created": "2026-07-03T11:02:08.208Z"
  },
  {
    "id": "mr4tq6hc8t4p",
    "text": "Monitorear respuesta SIMIT a DP del 02/07 (C29 Itagüí 0000838097)",
    "category": "auto",
    "done": false,
    "created": "2026-07-03T11:02:08.208Z"
  }
...
// Cargar el contexto de estudio
let studyContext = '';
if (fs.existsSync(CONTEXT_PATH)) {
  console.log('📚 Cargando tu contexto de estudio (ctx-estudio.md)...');
  studyContext = fs.readFileSync(CONTEXT_PATH, 'utf8');
  console.log(`✅ Contexto cargado (${(studyContext.length / 1024).toFixed(1)} KB)`);
} else {
  console.log('⚠️ No se encontró ctx-estudio.md. El asistente responderá sin contexto local.');
  console.log('💡 Consejo: Ejecuta "python scripts/generar_contexto.py" para generar tu material de estudio.');
}
...
async function preguntarGemini(prompt) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${API_KEY}`;
  
  // Construir el mensaje incluyendo el contexto de estudio en la primera interacción
  const systemInstructions = studyContext 
    ? `Eres un tutor de estudio inteligente para Jeiser. Utiliza el siguiente contexto del material de estudio (Tecnicatura/Laboratorios) para responder sus dudas, guiarlo en su aprendizaje y proponerle retos si te lo pide:\n\n[CONTEXTO DE ESTUDIO]\n${studyContext}\n\n`
    ...
    contents: [
      {
        parts: [
          ...
          ...chatHistory,
          { text: `Pregunta de Jeiser: ${prompt}` }
        ]
      }
    ]
  };
  ...
  try {
    ...
    return data.candidates?.[0]?.content?.parts?.[0]?.text || 'No obtuve respuesta del modelo.';
  } catch (error) {
...
function iniciarChat() {
  console.log('\n========================================================');
  console.log('🎓 BIENVENIDO A TU ASISTENTE DE ESTUDIO (Gemini 2.5 Flash)');
  console.log('Escribe tu duda o pide un examen. Escribe "salir" para terminar.');
  console.log('========================================================\n');

  const preguntar = () => {
    rl.question('👤 Jeiser > ', async (input) => {
      if (input.toLowerCase() === 'salir') {
        console.log('👋 ¡Buen trabajo de estudio hoy! Hasta luego.');
        rl.close();
        process.exit(0);
      }
      ...
      const respuesta = await preguntarGemini(input);
      console.log(`\n🤖 Tutor:\n${respuesta}\n`);

      // Guardar en el historial local para mantener la memoria de la conversación
      chatHistory.push({ text: `Jeiser: ${input}` });
      chatHistory.push({ text: `Tutor: ${respuesta}` });

      // Limitar historial para no saturar tokens (últimas 10 interacciones)
...
<file path=".github/workflows/brain_orchestrator.yml">
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: '22'
      - run: npm install
      - name: Restore Google OAuth files
        env:
      ...
      - name: Run brain orchestrator
        ...
        run: node scripts/brain_orchestrator.js
      - name: Commit state
        continue-on-error: true
        run: |
          git config user.name "brain-orchestrator"
          git config user.email "brain-orchestrator@users.noreply.github.com"
          git add .
          git diff --cached --quiet || git commit -m "chore(brain): persist orchestrator state"
          git push
</file>
...
<file path=".github/workflows/agent_heartbeat.yml">
...
jobs:
  heartbeat:
    ...
    steps:
      ...
      - name: Run inbox sensor
        run: node scripts/inbox_sensor.js
        env:
          TELEGRAM_BOT_TOKEN: ${{ secrets.TELEGRAM_BOT_TOKEN }}
          TELEGRAM_CHAT_ID: ${{ secrets.TELEGRAM_CHAT_ID }}
      ...
      - name: Commit processed state
        continue-on-error: true
        run: |
          git config user.name "agent-heartbeat"
          git config user.email "agent-heartbeat@users.noreply.github.com"
          git add data/processed_emails.json || true
          git diff --cached --quiet || git commit -m "chore(heartbeat): persist inbox sensor state"
          git push
</file>

<file path=".github/workflows/daily_alert.yml">
name: Daily Telegram Alert

on:
  schedule:
    # 12:00 UTC es 7:00 AM COT (Hora de Colombia)
    - cron: '0 12 * * *'
  ...
  notify:
    ...
    steps:
      ...
      - name: Commit state
        continue-on-error: true
        run: |
          git config user.name "daily-alert"
          git config user.email "daily-alert@users.noreply.github.com"
          git add .
          git diff --cached --quiet || git commit -m "chore(daily): persist alert state"
          git push
</file>
...
if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID) {
  console.warn('⚠️ TELEGRAM_BOT_TOKEN o TELEGRAM_CHAT_ID no están definidos. El bot no se iniciará.');
}
...
if (bot) {
  ...
  bot.launch().catch(err => console.error('❌ Error lanzando bot Telegram:', err.message));
  console.log('🤖 Bot de Telegram (Telegraf) iniciado y escuchando comandos.');
...
const LLM_PROVIDERS = [
  { name: 'Ollama Local (Llama 3.1)', baseUrl: process.env.OLLAMA_HOST || 'http://127.0.0.1:11434/v1', apiKey: 'ollama', model: 'llama3.1:latest', retry: 1 },
  { name: 'NVIDIA Nemotron 550B', baseUrl: process.env.NVIDIA_BASE_URL || 'https://integrate.api.nvidia.com/v1', apiKey: process.env.NVIDIA_API_KEY, model: process.env.NVIDIA_MODEL || 'nvidia/nemotron-3-ultra-550b-a55b', retry: 1 },
  { name: 'Cerebras', baseUrl: process.env.CEREBRAS_BASE_URL || 'https://api.cerebras.ai/v1', apiKey: process.env.CEREBRAS_API_KEY, model: process.env.CEREBRAS_MODEL || 'cerebras-llama-3.3-70b' },
  { name: 'OpenRouter Free', baseUrl: process.env.OPENROUTER_BASE_URL || 'https://openrouter.ai/api/v1', apiKey: process.env.OPENROUTER_API_KEY, model: process.env.OPENROUTER_MODEL || 'nousresearch/hermes-3-llama-3.1-405b:free' }
].filter(p => p.baseUrl && p.model);
  ...
  for (const provider of LLM_PROVIDERS) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        log(`ðŸ“¡ [${provider.name}] Llamando a ${url} (modelo: ${provider.model})`);
        ...
        if (!res.ok) {
          ...
          if (attempt < maxAttempts - 1 && (res.status === 429 || res.status >= 500)) {
            const delay = Math.pow(2, attempt + 1) * 1000;
            log(`âš ï¸ [${provider.name}] SaturaciÃ³n (${res.status}), reintentando en ${delay/1000}s... (${attempt + 1}/${maxAttempts - 1})`);
            await new Promise(r => setTimeout(r, delay));
            continue;
          }
          lastError[provider.name] = `${res.status}: ${errText}`;
          log(`âš ï¸ [${provider.name}] FallÃ³ (${res.status}), probando siguiente...`);
          break;
        }
        ...
        const content = data.choices?.[0]?.message?.content;
        if (content) {
          log(`âœ… [${provider.name}] Respuesta exitosa.`);
          return content;
        }
        lastError[provider.name] = 'Respuesta vacÃ­a';
        break;
      } catch (err) {
        if (attempt < maxAttempts - 1) {
          const delay = Math.pow(2, attempt + 1) * 1000;
          log(`âš ï¸ [${provider.name}] Error de red, reintentando en ${delay/1000}s... (${attempt + 1}/${maxAttempts - 1})`);
          await new Promise(r => setTimeout(r, delay));
          continue;
        }
        lastError[provider.name] = err.message;
        log(`âš ï¸ [${provider.name}] Error: ${err.message}, probando siguiente...`);
        break;
      }
    }
  }
...
async function run() {
  ...
  try {
    ...
    const sshIP = detectTailscaleIP();
    const sshMsg = `ðŸ”‘ <b>ACCESO REMOTO SSH ACTIVO</b>\n\nâ€¢ Servidor: <code>${escapeHTML(sshIP)}</code>\nâ€¢ Usuario: <code>dev</code>\nâ€¢ Puerto: <code>22</code>\nâ€¢ InstrucciÃ³n: Usa este host desde tu S23 Ultra vÃ­a SSH para tomar notas en ESTADO_VIVO.md.`;
    await sendTelegramMessage(sshMsg).catch(e => log(`âš ï¸ Error enviando notificaciÃ³n SSH: ${e.message}`));
