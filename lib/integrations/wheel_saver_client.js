...
const VENV_PYTHON = process.platform === 'win32'
  ? path.join(WHEELSAVER_DIR, 'venv', 'Scripts', 'python.exe')
  : path.join(WHEELSAVER_DIR, 'venv', 'bin', 'python');
const CLI_SCRIPT = path.join(WHEELSAVER_DIR, 'cli.py');  const DEFAULT_API_PORT = 8008;
  const API_BASE = `http://127.0.0.1:${DEFAULT_API_PORT}`;

  let _isOperationalCached = null;
 ...
 */  async function startServer(opts = {}) {
    const inst = checkInstallation();
    if (!inst.operational) {
      console.warn('[WheelSaver] 🚫 Base de datos vacía o inactiva. Cancelando inicio de servidor API.');
      return false;
    }
 ...
 */  async function search(query, opts = {}) {
    const inst = checkInstallation();
    if (!inst.operational) {
      console.warn('[WheelSaver] 🚫 Base de datos vacía o inactiva. Retornando búsqueda vacía.');
      return [];
    }
 ...
 */  async function stats() {
    const inst = checkInstallation();
    if (!inst.operational) {
      console.warn('[WheelSaver] 🚫 Base de datos vacía. Retornando stats vacío.');
      return [];
    }
    try {
...
}  // ── Verificar instalación (con caché operacional) ─────────────────
  function checkInstallation() {
    const fs = require('node:fs');
    const dbPath = path.join(WHEELSAVER_DIR, 'data', 'top_repos.db');
    const dbExists = fs.existsSync(dbPath);

    if (_isOperationalCached !== null) {
      const ok = fs.existsSync(WHEELSAVER_DIR) && fs.existsSync(VENV_PYTHON) && fs.existsSync(CLI_SCRIPT) && dbExists;
      return {
        dir: fs.existsSync(WHEELSAVER_DIR),
        venv: fs.existsSync(VENV_PYTHON),
        cli: fs.existsSync(CLI_SCRIPT),
        db: dbExists,
        dbHasData: _isOperationalCached,
        ok,
        operational: ok && _isOperationalCached,
      };
    }

    // FIX-015: Verificar si la DB tiene datos
    let dbHasData = false;
    if (dbExists) {
      try {
        const Database = require('better-sqlite3');
        const db = new Database(dbPath, { readonly: true });
        const row = db.prepare("SELECT COUNT(*) as count FROM repos").get();
        dbHasData = (row && row.count > 0);
        db.close();
      } catch {
        // Si la tabla no existe o hay otro error, asumir vacía
        dbHasData = false;
      }
    }

    _isOperationalCached = dbHasData;
    ...
    checks.operational = checks.ok && dbHasData;
    return checks;
  }
