...
const SCHEME_BEFORE = {
  ...
  Martes: ['1', '4'],
  Miercoles: ['2', '0'],
  Jueves: ['5', '9'], // Particulares (6 era solo para taxis)
  Viernes: ['7', '9'],
};
...
async function scrapePyPHoy() {
  try {
    const cheerio = require('cheerio');
    const res = await fetch('https://www.pyphoy.com/medellin', {
    ...
    let particularesDigits = [];
    $('div').each((_, el) => {
      const text = $(el).text().replace(/\s+/g, ' ').trim();
      if (text.includes('Particulares') && text.includes('placas terminadas en')) {
        const matchDigits = text.match(/placas terminadas en\s*([\d\s\-,]+)/i);
        if (matchDigits) {
          particularesDigits = matchDigits[1].replace(/[^\d]/g, '').split('');
        }
      }
    });

    if (particularesDigits.length > 0) {
      return particularesDigits;
    }
  } catch (e) {
    // Fallback silencioso a reglas deterministas
  }
  return null;
}

/**
 * Retorna estado de Pico y Placa priorizando scraping en vivo desde pyphoy.com/medellin
 */
async function getLivePicoYPlacaStatus(date = getColombiaNow()) {
  const liveDigits = await scrapePyPHoy();
  const name = dayName(date);
  const placa = getPlateDigit();

  if (liveDigits && liveDigits.length > 0) {
    const appliesToday = liveDigits.includes(placa);
    const hours = getHours();
    const nowM = minutesNow(date);
    const inWindow = nowM >= parseHm(hours.start) && nowM < parseHm(hours.end);
    const restrictedNow = appliesToday && inWindow;
    ...
    if (name === 'Sabado' || name === 'Domingo') {
      ...
      message = `✅ Sin pico y placa hoy (${name}) — placa …${placa} libre (pyphoy.com: ${liveDigits.join(' y ')})`;
    } else if (restrictedNow) {
      message = `🚫 *PICO Y PLACA AHORA (pyphoy.com)* — placa …${placa} | ${name} ${hours.start}–${hours.end}. NO saques el Corolla.`;
    } else {
      message = `⚠️ Pico y placa *hoy (pyphoy.com)* (${name}) placa …${placa}, ventana ${hours.start}–${hours.end}. Hoy restringen: ${liveDigits.join(' y ')}.`;
    }
    ...
    return {
      ...
      restrictedNow,
      hours,
      source: 'https://www.pyphoy.com/medellin',
      message,
    };
  }

  return getPicoYPlacaStatus(date);
}

module.exports = {
  getPicoYPlacaStatus,
  getLivePicoYPlacaStatus,
  scrapePyPHoy,
  getPlateDigit,
  getSchemeForDate,
  SCHEME_START,
