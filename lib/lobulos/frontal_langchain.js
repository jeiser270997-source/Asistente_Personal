...
const { createLLM } = require('../ai/litellm_client');
...
class LobuloFrontalLangChain {
  ...
  async _init() {
    // 1. Intentar LiteLLM proxy (smart-router con fallback DeepSeek → Gemini)
    try {
      const llm = await createLLM({ temperature: 0.1, maxTokens: 2000 });
      if (llm) {
        console.log('[Frontal] LLM iniciado vía LiteLLM (smart-router)');
        this.llm = llm;
        this._buildAgent();
        return;
      }
    } catch (err) {
      console.warn('[Frontal] LiteLLM falló:', err.message);
    }
