...
function matchAll(email) {
  ...
  // Collect matches synchronously (json-rules-engine v7 runs rules via engine.run)
  // We need to process results as they come
  const pending = [];
  ...
  // engine.run() returns a promise in v7
  // We use a sync approach: addRule with conditions, then evaluate each fact
  // Actually, json-rules-engine v7 engine.run() is async, so we must adapt
  // For sync compatibility, evaluate conditions manually
  try {
    // Manual synchronous evaluation for backward compatibility
    for (const rule of rules) {
      if (rule.enabled === false) continue;
    ...
    console.error('[rule_engine] Error evaluating rules:', e.message);
  }
...
module.exports = { matchAll, highestPriority, loadRules, createEngine };
