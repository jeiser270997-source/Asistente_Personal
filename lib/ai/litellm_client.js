 ...
 * cae automáticamente a OpenRouter API.
 *
...
/**
 * Crea un ChatOpenAI apuntando a OpenRouter.
 */
function buildOpenRouter(temperature = 0.1, maxTokens = 2000) {
  return new ChatOpenAI({
...
/**
 * Crea un ChatOpenAI apuntando a Groq (segundo fallback).
 */
function buildGroq(temperature = 0.1, maxTokens = 2000) {
  return new ChatOpenAI({
...
async function createLLM({ temperature = 0.1, maxTokens = 2000 } = {}) {
  ...
  if (process.env.OPENROUTER_API_KEY) {
    ...
    return buildOpenRouter(temperature, maxTokens);
  }

  // 3. Groq
  if (process.env.GROQ_API_KEY) {
    console.log('[LiteLLM] Groq activo');
    return buildGroq(temperature, maxTokens);
  }
...
module.exports = { createLLM, probeLiteLLM, buildLiteLLM, buildOpenRouter, buildGroq, LITELLM_URL };
