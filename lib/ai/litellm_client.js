...
async function probeLiteLLM(timeout = 1000) {
  ...
  const now = Date.now();
  if (now - lastProbeTime < 5 * 60 * 1000) {
...
async function getLLMClients(sensitive = false) {
  ...
  if (sensitive) {
    const localOnly = clients.filter(c => c._model === 'smart-router');
    if (localOnly.length === 0) {
      throw new Error('[SENSITIVE] Se requiere el proxy LiteLLM local para procesar contexto sensible, y no está disponible. Abortando en vez de usar un proveedor en la nube.');
    }
    return localOnly;
  }
