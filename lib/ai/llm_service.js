 ...
 * Usa LiteLLM como proxy primario con fallback automático a DeepSeek directo.
 *
 * Migrado de fetch() nativo → ChatOpenAI (LangChain) — Jul 2026.
 * LiteLLM integration — Jul 2026.
 */

require('dotenv').config({ path: require('node:path').join(__dirname, '..', '..', '.env') });
const { ChatOpenAI } = require('@langchain/openai');
const { createLLM } = require('./litellm_client');
...
async function askDeepSeek(systemPrompt, messages, temperature = 0.1) {
  ...
  // Try LiteLLM proxy first, fallback to direct DeepSeek
  const llm = await createLLM({ temperature, maxTokens: 2000 });
  if (!llm) {
    throw new Error('No se pudo crear LLM (sin API key)');
  }
