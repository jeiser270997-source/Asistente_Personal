  ...
  bot.command('nota', async (ctx) => {
    ...
    const timestamp = new Date().toLocaleString('es-CO', { timeZone: 'America/Bogota' });
      ...
      if (fs.existsSync(INBOX_DIR)) {
        ...
        // Extraer palabras clave del mensaje del usuario (más de 3 letras)
        ...
        if (files.length === 0) {
          ...
          files.forEach(f => {
            ...
            // ¿El usuario mencionó alguna palabra que esté en este archivo?
            const isRelevant = userWords.some(w => contentLower.includes(w));
            
            // Si es relevante, o si el usuario pide un resumen general, cargamos el contenido
            if (isRelevant || userMessageLower.includes('resumen') || userMessageLower.includes('inbox')) {
              inboxContext += '\n📄 ' + f + ':\n' + content.substring(0, 1500) + '\n';
            } else {
              // Si no es relevante, solo le decimos al LLM que el archivo existe
              inboxContext += '\n📄 ' + f + ' (Disponible en INBOX. El usuario no ha preguntado por esto, no cargado en memoria).\n';
            }
          });
        }
      }
