...
if (require.main === module && bot) {
  ...
  bot.start((ctx) => {
    ...
    return ctx.reply(
      '🤖 <b>Jarvis — Life OS</b>\n\nComandos:\n• /start — Menú\n• /tareas — Pendientes\n• /ping — Prueba\n\n💬 O simplemente escríbeme y te respondo con IA.',
      { parse_mode: 'HTML' }
    );
  });
  ...
  bot.on('text', async (ctx) => {
    ...
    try {
      ...
      const { askLLM } = require('./llm_service');
      const response = await askLLM(
        'Eres Jarvis, asistente personal de Jeiser. Responde en español, sé conciso (max 3-4 líneas). Conoces su contexto: vive en Colombia, estudia en CESDE, usa GitHub Actions para automatizar su vida.',
        [{ role: 'user', content: msg }]
      );
      const reply = response?.content || 'No pude procesar tu mensaje.';
      await ctx.reply(reply, { parse_mode: 'HTML' });
    } catch (e) {
      ctx.reply('💥 Error: ' + e.message.substring(0, 200));
    }
  });
