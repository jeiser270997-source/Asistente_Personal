...
const MEMORY_ENGINE_PATH = path.join(BASE_DIR, 'lib', 'memory', 'memory_engine.js');
...
function loadSenaContext() {
  const senaDir = path.join(DATA_DIR, 'sena');
  const cursoPath = path.join(senaDir, 'curso.json');
  const deadlinesPath = path.join(senaDir, 'deadlines.json');
  const partes = [];
