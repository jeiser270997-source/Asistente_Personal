...
Sé crítico. No infles scores.`;
  ...
  try {
    ...
    try {
      ...
      return { score: Math.round(maxWeight * 0.5), strengths: [], weaknesses: [], redFlags: [], reasoning: 'Error parseando respuesta LLM', tokensConsumed: 0 };
    }
    ...
    console.warn('[scorer] LLM fallback: no se pudo obtener evaluación LLM, usando score neutral');
    return { score: Math.round(maxWeight * 0.5), strengths: [], weaknesses: [], redFlags: [], reasoning: `LLM no disponible: ${e.message}`, tokensConsumed: 0 };
  }
}
