...
const PROVIDERS = [
  { name: 'DeepSeek V4 Flash', url: 'https://api.deepseek.com/v1/chat/completions', key: process.env.DEEPSEEK_API_KEY, models: ['deepseek-v4-flash'] },
...
async function askLLM(systemPrompt, messages, tools = [], temperature = 0.1) {
  const activeProviders = PROVIDERS.filter(p => p.key && p.key !== 'undefined');
